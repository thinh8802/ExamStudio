        ...
        <div className="space-y-2 z-10 max-w-2xl">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight text-[hsl(var(--foreground))] leading-snug">
              {greeting}, <span className="text-gradient-primary">{profile.nickname || profile.fullName || userName}</span> 👋
            </h1>

            {isLicensed && (
              <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                <ShieldCheck className="w-3.5 h-3.5" />
                <span>Bản quyền: <strong>{profile.fullName || userName}</strong></span>
                <span className="text-[10px] px-1.5 py-0.2 rounded-md bg-amber-500/15 text-amber-600 dark:text-amber-400 font-bold">
                  {payload?.type === 'lifetime' ? '👑 PRO Vĩnh Viễn' : '⭐ 1 Tháng VIP'}
                </span>
              </div>
            )}
          </div>
