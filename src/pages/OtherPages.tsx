...
import { useAppStore, type ShortcutConfig, DEFAULT_SHORTCUTS } from '@/stores/app-store';
...
import {
  ...
  RefreshCw, ArrowRight, ShieldCheck, Key, Keyboard
} from 'lucide-react';
...
import { useUserProfileStore, AVATAR_GRADIENTS, ROLE_LABELS, type UserRole } from '@/stores/user-profile-store';
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-12 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-5">
          ...
          {/* 4. Phím tắt thao tác nhanh (Customizable Keyboard Shortcuts) */}
        ...
        <div className="space-y-5">
          ...
          {/* 3.3. Hồ Sơ Cá Nhân & Biệt Danh */}
          <ProfileCustomizationCard />

          {/* 3.4. Bảo Mật & Mật Khẩu Đăng Nhập */}
          <SecurityPasswordCard />

          {/* 3.5. Bản Quyền Ứng Dụng (License Status) */}
          <LicenseInfoCard />

          {/* 3.6. Cập Nhật Ứng Dụng (Auto Update) */}
          <AutoUpdateCard />
...
const ProfileCustomizationCard: React.FC = () => {
  const { profile, setNickname, setRole, setAvatarColor, setProfile } = useUserProfileStore();
  const [editingNickname, setEditingNickname] = React.useState(profile.nickname || '');
  const [editingGoal, setEditingGoal] = React.useState(profile.studyGoal || '');
  const [isSaved, setIsSaved] = React.useState(false);

  React.useEffect(() => {
    setEditingNickname(profile.nickname || '');
    setEditingGoal(profile.studyGoal || '');
  }, [profile]);

  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    setNickname(editingNickname);
    setProfile({ studyGoal: editingGoal });
    setIsSaved(true);
    toast.success('Đã cập nhật hồ sơ cá nhân!');
    setTimeout(() => setIsSaved(false), 2000);
  };

  const firstLetter = (profile.nickname || profile.fullName || 'E').trim().charAt(0).toUpperCase();
    ...
    <Card className="rounded-3xl border border-[hsl(var(--border))] shadow-xs">
      <CardContent className="p-5 space-y-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-bold text-sm sm:text-base text-[hsl(var(--foreground))]">
                Hồ Sơ Cá Nhân & Biệt Danh
              </h2>
              <p className="text-xs text-[hsl(var(--muted-foreground))]">
                Tùy biến tên gọi thân mật và phong cách hiển thị trong ứng dụng.
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-3.5 text-xs">
          {/* Họ và tên bản quyền chính chủ (Khóa cố định) */}
          <div className="p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-between">
            <div>
              <div className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                <ShieldCheck size={12} />
                Bản Quyền Chính Chủ (Đã Xác Thực)
              </div>
              <div className="font-bold text-sm text-[hsl(var(--foreground))] mt-0.5">
                {profile.fullName || 'Người học ExamPrep'}
              </div>
              {profile.email && (
                <div className="text-[11px] text-[hsl(var(--muted-foreground))]">{profile.email}</div>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ví dụ: Thịnh Pro, Thầy Thịnh..."
              className="w-full px-3 py-2 rounded-xl bg-[hsl(var(--muted))] border border-[hsl(var(--border))] text-xs font-semibold text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary)/0.4)]"
            />
          </div>

          {/* Chọn Avatar Gradient */}
          <div>
            <label className="font-semibold text-[hsl(var(--foreground))] block mb-1.5">
              Avatar & Màu sắc phong cách:
            </label>
            <div className="flex items-center gap-2">
              <div
                className={cn(
                  'w-8 h-8 rounded-xl bg-gradient-to-tr flex items-center justify-center text-xs font-black text-white shadow-xs shrink-0',
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                {AVATAR_GRADIENTS.map((g) => (
                  <button
                    ...
                    className={cn(
                      'w-6 h-6 rounded-lg bg-gradient-to-tr cursor-pointer transition-all',
                      g.value,
                      profile.avatarColor === g.value
                        ? 'ring-2 ring-[hsl(var(--primary))] ring-offset-1 ring-offset-[hsl(var(--card))] scale-110'
              ...
              {(Object.keys(ROLE_LABELS) as UserRole[]).map((r) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setRole(r)}
                    className={cn(
                      'p-2 rounded-xl border text-center cursor-pointer transition-all text-xs font-semibold',
                      isSelected
                        ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/0.1)] text-[hsl(var(--primary))]'
                        : 'border-[hsl(var(--border))] bg-[hsl(var(--muted)/0.3)] hover:bg-[hsl(var(--muted)/0.6)] text-[hsl(var(--muted-foreground))]'
                    )}
                  >
                    <span>{item.icon} {item.label.split('/')[0].trim()}</span>
                  </button>
                );
              })}
            </div>
          ...
          >
            {isSaved ? '✓ Đã lưu thay đổi!' : 'Lưu thông tin hồ sơ'}
          </Button>
        </form>
      </CardContent>
    </Card>
...
const SecurityPasswordCard: React.FC = () => {
  const [isProtected, setIsProtected] = React.useState<boolean>(false);
  const [showPasswordModal, setShowPasswordModal] = React.useState(false);
  const [modalMode, setModalMode] = React.useState<'set' | 'disable'>('set');
  ...
  const checkStatus = async () => {
    const protectedState = await authService.isPasswordProtected();
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (modalMode === 'disable') {
      const res = await authService.disablePassword(currentPasswordInput);
      setLoading(false);
      if (res.success) {
        toast.success('Đã tắt mật khẩu! Ứng dụng sẽ mở trực tiếp không cần mật khẩu.');
        setShowPasswordModal(false);
        checkStatus();
      } else {
        setError(res.error || 'Mật khẩu hiện tại không đúng');
      }
    } else {
      // Set new password
      ...
      if (isProtected) {
        const res = await authService.changePassword(currentPasswordInput, newPasswordInput);
        setLoading(false);
        if (res.success) {
        ...
        const username = await authService.getOwnerUsername();
        const res = await authService.setupAccount(username, newPasswordInput);
        setLoading(false);
        if (res.success) {
          toast.success('Đã bật mật khẩu bảo vệ thành công!');
          setShowPasswordModal(false);
          checkStatus();
        } else {
          setError(res.error || 'Lỗi khi cài đặt mật khẩu');
        }
      }
    }
  };
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className={cn(
                ...
                isProtected ? 'bg-amber-500/15 text-amber-500' : 'bg-emerald-500/15 text-emerald-500'
              )}>
                {isProtected ? <Lock size={16} /> : <ShieldCheck size={16} />}
              </div>
              <div>
                <h2 className="font-bold text-sm sm:text-base text-[hsl(var(--foreground))]">
                ...
                <p className="text-xs text-[hsl(var(--muted-foreground))]">
                  Tùy chọn mở app trực tiếp hoặc khóa màn hình bằng mật khẩu.
                </p>
              </div>
            </div>
            ...
            <span className={cn(
              ...
              {isProtected ? '🔒 Đang Bật Mật Khẩu' : '🔓 Không Cần Mật Khẩu'}
            </span>
          </div>

          <div className="p-3.5 rounded-2xl bg-[hsl(var(--muted)/0.3)] border border-[hsl(var(--border))] text-xs space-y-1.5">
            <div className="font-semibold text-[hsl(var(--foreground))]">
              {isProtected
                ? 'Ứng dụng đang yêu cầu nhập mật khẩu mỗi khi khởi động.'
                : 'Ứng dụng sẽ mở thẳng vào màn hình chính nhanh chóng mà không cần nhập mật khẩu.'}
            </div>
            <p className="text-[11px] text-[hsl(var(--muted-foreground))] leading-relaxed">
              Bạn có thể tự do tắt hoặc bật mật khẩu bảo vệ bất cứ lúc nào tùy theo nhu cầu sử dụng cá nhân.
            </p>
          </div>
          ...
          <div className="flex gap-2 pt-1">
            {isProtected ? (
              ...
              <Button
                variant="outline"
                size="sm"
                className="w-full text-xs font-bold cursor-pointer text-[hsl(var(--primary))] border-[hsl(var(--primary)/0.3)] bg-[hsl(var(--primary)/0.05)] hover:bg-[hsl(var(--primary)/0.1)]"
                onClick={handleOpenSet}
              >
                🔒 Bật mật khẩu bảo vệ ứng dụng
              </Button>
            )}
          </div>
        </CardContent>
      ...
      {/* Modal Cài đặt / Tắt Mật khẩu */}
      <Modal
        isOpen={showPasswordModal}
        onClose={() => setShowPasswordModal(false)}
        title={
          modalMode === 'disable'
            ? 'Tắt Mật Khẩu Mở Khóa'
            : isProtected
            ? 'Đổi Mật Khẩu Mới'
            : 'Tạo Mật Khẩu Bảo Vệ Mới'
        }
        size="md"
      >
        <form onSubmit={handleFormSubmit} className="space-y-4">
          <p className="text-xs text-[hsl(var(--muted-foreground))]">
            {modalMode === 'disable'
              ? 'Nhập mật khẩu hiện tại để xác nhận tắt tính năng khóa màn hình.'
              : 'Thiết lập mật khẩu an toàn (tối thiểu 6 ký tự) để bảo vệ không gian học tập của bạn.'}
          </p>

          {/* Nhập mật khẩu hiện tại nếu cần */}
          {(modalMode === 'disable' || (modalMode === 'set' && isProtected)) && (
            <div>
          ...
          {/* Nhập mật khẩu mới nếu ở chế độ set */}
          {modalMode === 'set' && (
            <>
              <div>
                <label className="text-xs font-semibold text-[hsl(var(--foreground))] block mb-1">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              type="submit"
              variant={modalMode === 'disable' ? 'destructive' : 'default'}
              size="sm"
              disabled={loading}
            >
              {loading ? 'Đang xử lý...' : modalMode === 'disable' ? 'Xác Nhận Tắt Mật Khẩu' : 'Lưu Mật Khẩu'}
            </Button>
          </div>
        </form>
      </Modal>
...
const LicenseInfoCard: React.FC = () => {
  const { payload, rawKey, activatedAt, expiresAt, activateLicense } = useLicenseStore();
  ...
  // Tính toán nhãn loại bản quyền chính xác
  const getPlanName = () => {
    if (payload?.type === 'subscription') {
      if (payload.dur && payload.dur > 0) {
        if (payload.dur < 86400) {
          return `⏱️ Thử nghiệm ${Math.round(payload.dur / 60)} Phút`;
        }
        return `📅 Có thời hạn (${Math.round(payload.dur / 86400)} Ngày)`;
      }
      if (payload.exp && payload.exp > 0) {
        return `📅 Có thời hạn (Đến ${new Date(payload.exp * 1000).toLocaleDateString('vi-VN')})`;
      }
      return '📅 Bản quyền có thời hạn';
    }
    return '⭐ VĨNH VIỄN (Lifetime Pro)';
  };
          ...
          <div className="p-3.5 rounded-2xl bg-[hsl(var(--muted)/0.25)] border border-[hsl(var(--border))] text-xs space-y-2">
            ...
            {activatedAt && (
              <div className="flex items-center justify-between text-[11px] text-[hsl(var(--muted-foreground))]">
                ...
                <span className="font-mono">{new Date(activatedAt).toLocaleString('vi-VN')}</span>
              </div>
            )}
          ...
          {status === 'available' ? (
            <div className="space-y-2">
              <Button
                ...
                className="w-full justify-center bg-[hsl(var(--primary))] text-white font-bold text-xs cursor-pointer shadow-xs"
              >
                Tải bản cập nhật v{updateInfo?.version} ngay
              </Button>
              <button
                type="button"
                onClick={() => window.open('https://github.com/thinh8802/ExampStudio/releases/latest', '_blank')}
                className="w-full flex items-center justify-center gap-1.5 text-[11px] font-medium text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))] transition-colors cursor-pointer py-1"
              >
                <ExternalLink size={12} />
                <span>Hoặc tải trực tiếp file Setup từ GitHub Releases</span>
              </button>
            </div>
          ) : status === 'downloaded' ? (
            <Button
              ...
              className="w-full justify-center bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs cursor-pointer shadow-xs"
            >
            ...
            <div className="space-y-2">
              <Button
                ...
                icon={<Download size={14} />}
                onClick={() => window.open('https://github.com/thinh8802/ExampStudio/releases/latest', '_blank')}
                className="w-full justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs cursor-pointer shadow-xs"
              >
                Tải file Setup v3.3.2+ trực tiếp từ GitHub Releases
              </Button>
              <Button
                variant="outline"
                size="sm"
            ...
            <div className="space-y-1.5">
              ...
              <div className="text-center">
                <button
                  type="button"
                  onClick={() => window.open('https://github.com/thinh8802/ExampStudio/releases', '_blank')}
                  className="inline-flex items-center gap-1 text-[11px] text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))] transition-colors cursor-pointer"
                >
                  <ExternalLink size={11} />
                  <span>Xem lịch sử phát hành trên GitHub</span>
                </button>
              </div>
            </div>
          )}
...
const CustomGradientModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Bảng Tùy Chỉnh Màu Sắc & Gradient Tự Do" size="lg">
      <div className="space-y-4 sm:space-y-5 p-0.5">
        ...
        <div className="p-4 sm:p-5 rounded-2xl sm:rounded-3xl border-2 border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm space-y-3.5 sm:space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-[hsl(var(--foreground))] flex items-center gap-2">
              <Sparkles size={16} className="text-amber-500" /> Bản Xem Trước Trực Quan
            </span>
            <span className="text-[11px] font-mono font-bold px-2.5 py-1 rounded-full bg-[hsl(var(--muted))] text-[hsl(var(--foreground))] border-2 border-[hsl(var(--border))] shadow-2xs">
              Góc xoay: {angle}°
            </span>
          </div>
          ...
          {/* Gradient Banner */}
          <div
            className="w-full h-11 sm:h-12 rounded-xl sm:rounded-2xl shadow-md flex items-center justify-center text-white font-extrabold text-xs tracking-wider transition-all duration-300 border border-white/30 px-2 text-center"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3 pt-1">
            <button
              type="button"
              className="px-3 py-2.5 rounded-xl text-white font-bold text-xs shadow-md flex items-center justify-center gap-1.5 transition-transform active:scale-95 cursor-pointer border border-white/30"
              style={{ background: currentGradStyle }}
            >
            ...
            <div
              className="px-3 py-2.5 rounded-xl border-2 text-xs font-bold flex items-center justify-center gap-1.5 shadow-2xs"
              style={{ borderColor: color1, color: color1, backgroundColor: `${color1}18` }}
            >
            ...
            <div
              className="px-3 py-2.5 rounded-xl border-2 text-xs font-bold flex items-center justify-center gap-1.5 shadow-2xs"
              style={{ borderColor: color2, color: color2, backgroundColor: `${color2}18` }}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
          {/* Color 1 */}
          <div className="p-3.5 sm:p-4 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-[hsl(var(--foreground))] flex items-center gap-2">
                <div className="w-4 h-4 rounded-full shadow-xs border border-white/40 ring-1 ring-black/10" style={{ backgroundColor: color1 }} />
                <span>Màu Bắt Đầu (Đầu 1)</span>
              </label>
              <span className="text-xs font-mono font-bold px-2.5 py-0.5 rounded-lg bg-[hsl(var(--muted))] text-[hsl(var(--foreground))] border-2 border-[hsl(var(--border))]">
                {color1.toUpperCase()}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <div className="relative">
                <input
                  ...
                  className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl cursor-pointer border-2 border-[hsl(var(--border))] bg-[hsl(var(--muted)/0.3)] p-1 shadow-2xs hover:scale-105 transition-transform"
                />
              </div>
              <input
                type="text"
                value={color1}
                onChange={e => setColor1(e.target.value)}
                className="flex-1 px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-xl text-xs font-mono font-semibold bg-[hsl(var(--background))] border-2 border-[hsl(var(--border))] text-[hsl(var(--foreground))] focus:outline-none focus:border-[hsl(var(--primary))] focus:ring-2 focus:ring-[hsl(var(--primary)/0.2)] transition-all shadow-2xs"
                placeholder="#4f46e5"
              />
            </div>
            ...
            {/* Quick Swatches 1 */}
            <div className="space-y-1.5 pt-1">
              <span className="text-[11px] font-semibold text-[hsl(var(--muted-foreground))]">Bảng màu gợi ý:</span>
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {SWATCHES_1.map(sw => {
                  const isSelected = color1.toLowerCase() === sw.color.toLowerCase();
                  return (
                    <button
                      key={sw.color}
                      type="button"
                      onClick={() => setColor1(sw.color)}
                      className={`w-6 h-6 sm:w-7 sm:h-7 rounded-xl shadow-xs border-2 transition-all hover:scale-115 cursor-pointer ${
                        isSelected 
                          ? 'border-[hsl(var(--foreground))] ring-2 ring-[hsl(var(--primary))] scale-110' 
          ...
          <div className="p-3.5 sm:p-4 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-[hsl(var(--foreground))] flex items-center gap-2">
                <div className="w-4 h-4 rounded-full shadow-xs border border-white/40 ring-1 ring-black/10" style={{ backgroundColor: color2 }} />
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {SWATCHES_2.map(sw => {
                  const isSelected = color2.toLowerCase() === sw.color.toLowerCase();
                  return (
                    <button
                      key={sw.color}
                      type="button"
        ...
        <div className="p-3.5 sm:p-4 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-3">
          <div className="flex justify-between items-center text-xs">
            <span className="font-bold text-[hsl(var(--foreground))] flex items-center gap-1.5">
              <span>📐</span> Góc Xoay Gradient (Angle)
            </span>
            <span className="font-mono font-bold px-2.5 py-0.5 rounded-lg bg-[hsl(var(--primary)/0.1)] text-[hsl(var(--primary))] border-2 border-[hsl(var(--primary)/0.3)] shadow-2xs">
              {angle}°
            </span>
          </div>
          ...
          <div className="py-1">
            <input
              ...
              className="w-full h-2.5 bg-[hsl(var(--muted))] rounded-lg appearance-none cursor-pointer accent-[hsl(var(--primary))]"
            />
          </div>

          {/* 5 Evenly-spaced Mathematical Ticks: 0% -> 25% -> 50% -> 75% -> 100% */}
          <div className="flex justify-between text-[10px] sm:text-[10.5px] font-medium text-[hsl(var(--muted-foreground))]">
            <span className="w-12 text-left">0° (Dọc)</span>
            <span className="text-center">90° (Ngang)</span>
            <span className="text-center">180° (Ngược)</span>
            <span className="text-center">270° (Đảo)</span>
            <span className="w-12 text-right">360°</span>
          </div>

          {/* Quick Angle Preset Chips */}
          <div className="flex items-center gap-2 pt-1 border-t border-[hsl(var(--border)/0.5)]">
            <span className="text-[11px] font-semibold text-[hsl(var(--muted-foreground))]">Góc mẫu:</span>
            <div className="flex flex-wrap gap-1.5 flex-1">
              {[
                { label: '0° Dọc', val: 0 },
                { label: '45° Chéo', val: 45 },
                { label: '90° Ngang', val: 90 },
                { label: '✨ 135° Chuẩn', val: 135 },
                { label: '180° Xuôi', val: 180 },
                { label: '270° Đảo', val: 270 },
              ].map(item => (
                <button
                  key={item.val}
                  type="button"
                  onClick={() => setAngle(item.val)}
                  className={`px-2 py-0.5 rounded-lg text-[10.5px] font-bold border transition-all cursor-pointer shadow-2xs ${
                    angle === item.val
                      ? 'bg-[hsl(var(--primary))] text-white border-[hsl(var(--primary))] shadow-xs scale-105'
        ...
        {/* Action Buttons with Clear Frame & Borders */}
        <div className="flex items-center justify-between pt-3.5 border-t-2 border-[hsl(var(--border))]">
          <button
            type="button"
            onClick={handleReset}
            className="px-3.5 py-2 rounded-xl text-xs font-bold border-2 border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))] transition-all cursor-pointer shadow-2xs active:scale-95 flex items-center gap-1.5"
          >
            <span>🔄</span>
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            <button
              type="button"
              onClick={onClose}
              className="px-3.5 py-2 rounded-xl text-xs font-bold border-2 border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))] transition-all cursor-pointer shadow-2xs active:scale-95"
            >
              Hủy
            </button>
            <button
              type="button"
              onClick={handleApply}
              className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-[hsl(var(--primary))] hover:opacity-90 transition-all cursor-pointer shadow-md active:scale-95 flex items-center gap-1.5 border border-white/20"
            >
              <Sparkles size={14} />
...
/* ============================================
   CUSTOMIZABLE KEYBOARD SHORTCUTS MODAL & CARD
   ============================================ */
interface ShortcutCustomizerModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const ShortcutCustomizerModal: React.FC<ShortcutCustomizerModalProps> = ({ isOpen, onClose }) => {
  const { shortcuts, setShortcuts, resetShortcuts } = useAppStore();
  const [form, setForm] = React.useState<ShortcutConfig>(shortcuts);
  ...
  const handleApplyPreset = (preset: ShortcutConfig) => {
    setForm(preset);
  };

  const handleSave = () => {
    setShortcuts(form);
    toast.success('💾 Đã lưu cấu hình phím tắt mới!');
    onClose();
  };

  const handleReset = () => {
    resetShortcuts();
    setForm(DEFAULT_SHORTCUTS);
    toast.success('🔄 Đã khôi phục phím tắt mặc định!');
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Tùy Chỉnh Phím Tắt Thao Tác Nhanh" size="lg">
      <div className="space-y-4 sm:space-y-5 p-0.5">
        <p className="text-xs text-[hsl(var(--muted-foreground))] leading-relaxed">
          Tùy biến các nút bấm khi làm bài trắc nghiệm (chọn đáp án, chuyển câu, đánh dấu và xác nhận) để phù hợp nhất với thói quen của bạn.
        </p>

        {/* Quick Presets */}
        <div className="p-3.5 sm:p-4 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-2.5">
          <span className="text-xs font-bold text-[hsl(var(--foreground))] flex items-center gap-1.5">
            <Sparkles size={15} className="text-amber-500" /> Cấu hình mẫu phổ biến (Presets):
          </span>
            ...
            {[
              {
                name: '🏆 Mặc định',
                desc: '1-4 & A-D, N / P',
                cfg: { answerKeys: 'both', nextKey: 'N', prevKey: 'P', markKey: 'Space', confirmKey: 'Enter' } as ShortcutConfig,
              },
              {
                name: '➡️ Phím Mũi Tên',
                desc: 'Tiếp: → / Lùi: ←',
                cfg: { answerKeys: 'both', nextKey: 'ArrowRight', prevKey: 'ArrowLeft', markKey: 'Space', confirmKey: 'Enter' } as ShortcutConfig,
              },
              {
                name: '🎮 WASD Game',
                desc: 'Tiếp: D / Lùi: A',
                cfg: { answerKeys: '1-4', nextKey: 'D', prevKey: 'A', markKey: 'Space', confirmKey: 'Enter' } as ShortcutConfig,
              },
              {
                name: '🔤 Chỉ Chữ Cái',
                desc: 'A, B, C, D',
                cfg: { answerKeys: 'a-d', nextKey: 'N', prevKey: 'P', markKey: 'Space', confirmKey: 'Enter' } as ShortcutConfig,
              },
            ].map(p => (
              <button
                key={p.name}
                type="button"
                onClick={() => handleApplyPreset(p.cfg)}
                className="p-2.5 rounded-xl border-2 border-[hsl(var(--border))] bg-[hsl(var(--muted)/0.2)] hover:border-[hsl(var(--primary))] hover:bg-[hsl(var(--primary)/0.05)] text-left transition-all cursor-pointer shadow-2xs group active:scale-95"
              >
                <div className="font-bold text-xs text-[hsl(var(--foreground))] group-hover:text-[hsl(var(--primary))]">{p.name}</div>
                <div className="text-[10px] text-[hsl(var(--muted-foreground))] mt-0.5 font-medium">{p.desc}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3.5">
          ...
          <div className="p-3.5 sm:p-4 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-2.5">
            <label className="text-xs font-bold text-[hsl(var(--foreground))] flex items-center justify-between">
              <span>🔘 Phím chọn đáp án trắc nghiệm:</span>
              <span className="text-[10.5px] font-mono text-[hsl(var(--primary))] font-bold px-2 py-0.5 rounded-md bg-[hsl(var(--primary)/0.1)] border border-[hsl(var(--primary)/0.2)]">
                {form.answerKeys === 'both' ? 'Cả 1-4 & A-D' : form.answerKeys === '1-4' ? 'Chỉ 1, 2, 3, 4' : 'Chỉ A, B, C, D'}
              </span>
            </label>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {[
                { id: 'both', label: '🔢 + 🔤 Cả 1-4 & A-D' },
                { id: '1-4', label: '🔢 Chỉ phím 1, 2, 3, 4' },
                { id: 'a-d', label: '🔤 Chỉ phím A, B, C, D' },
              ].map(opt => (
                <button
                  key={opt.id}
                  type="button"
                  onClick={() => setForm({ ...form, answerKeys: opt.id as any })}
                  className={`p-2.5 rounded-xl text-xs font-bold border-2 transition-all cursor-pointer shadow-2xs text-center active:scale-95 ${
                    form.answerKeys === opt.id
                      ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/0.12)] text-[hsl(var(--primary))] ring-1 ring-[hsl(var(--primary))]'
          ...
          {/* Navigation & Action Keys Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {/* Next Key */}
            <div className="p-3.5 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-[hsl(var(--foreground))]">➡️ Câu tiếp theo (Next):</label>
                <kbd className="px-2 py-0.5 rounded-lg bg-[hsl(var(--muted))] text-xs font-mono font-bold border border-[hsl(var(--border))]">
                  {form.nextKey === 'ArrowRight' ? '→' : form.nextKey.toUpperCase()}
                </kbd>
              </div>
              <select
                value={form.nextKey}
                onChange={e => setForm({ ...form, nextKey: e.target.value })}
                className="w-full p-2.5 rounded-xl text-xs font-bold bg-[hsl(var(--background))] border-2 border-[hsl(var(--border))] text-[hsl(var(--foreground))] focus:outline-none focus:border-[hsl(var(--primary))] cursor-pointer shadow-2xs"
              >
                <option value="N">Phím N (Next - Mặc định)</option>
                <option value="ArrowRight">Phím Mũi Tên Phải (→)</option>
                <option value="D">Phím D (Phong cách Game WASD)</option>
                <option value="J">Phím J (Phong cách Vim)</option>
                <option value="L">Phím L</option>
            ...
            <div className="p-3.5 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-[hsl(var(--foreground))]">⬅️ Câu trước đó (Prev):</label>
                <kbd className="px-2 py-0.5 rounded-lg bg-[hsl(var(--muted))] text-xs font-mono font-bold border border-[hsl(var(--border))]">
                  {form.prevKey === 'ArrowLeft' ? '←' : form.prevKey.toUpperCase()}
                </kbd>
              </div>
              <select
                value={form.prevKey}
                onChange={e => setForm({ ...form, prevKey: e.target.value })}
                className="w-full p-2.5 rounded-xl text-xs font-bold bg-[hsl(var(--background))] border-2 border-[hsl(var(--border))] text-[hsl(var(--foreground))] focus:outline-none focus:border-[hsl(var(--primary))] cursor-pointer shadow-2xs"
              >
                <option value="P">Phím P (Previous - Mặc định)</option>
                <option value="ArrowLeft">Phím Mũi Tên Trái (←)</option>
                <option value="A">Phím A (Phong cách Game WASD)</option>
                <option value="K">Phím K (Phong cách Vim)</option>
                <option value="H">Phím H</option>
              </select>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-[hsl(var(--foreground))]">🚩 Đánh dấu câu (Flag):</label>
                <kbd className="px-2 py-0.5 rounded-lg bg-[hsl(var(--muted))] text-xs font-mono font-bold border border-[hsl(var(--border))]">
                  {form.markKey}
                </kbd>
              </div>
              <select
                value={form.markKey}
                onChange={e => setForm({ ...form, markKey: e.target.value })}
                className="w-full p-2.5 rounded-xl text-xs font-bold bg-[hsl(var(--background))] border-2 border-[hsl(var(--border))] text-[hsl(var(--foreground))] focus:outline-none focus:border-[hsl(var(--primary))] cursor-pointer shadow-2xs"
              >
                <option value="Space">Phím Space (Phím cách - Mặc định)</option>
                <option value="M">Phím M (Mark)</option>
                <option value="F">Phím F (Flag)</option>
                <option value="Q">Phím Q</option>
              </select>
            </div>

            {/* Confirm / Next in Practice */}
            <div className="p-3.5 rounded-2xl bg-[hsl(var(--card))] border-2 border-[hsl(var(--border))] shadow-xs space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-[hsl(var(--foreground))]">✅ Xác nhận câu (Luyện tập):</label>
                <kbd className="px-2 py-0.5 rounded-lg bg-[hsl(var(--muted))] text-xs font-mono font-bold border border-[hsl(var(--border))]">
                  {form.confirmKey}
                </kbd>
              </div>
              <select
                ...
                onChange={e => setForm({ ...form, confirmKey: e.target.value })}
                className="w-full p-2.5 rounded-xl text-xs font-bold bg-[hsl(var(--background))] border-2 border-[hsl(var(--border))] text-[hsl(var(--foreground))] focus:outline-none focus:border-[hsl(var(--primary))] cursor-pointer shadow-2xs"
              >
                <option value="Enter">Phím Enter (Mặc định)</option>
                <option value="C">Phím C (Confirm)</option>
                <option value="Space">Phím Space</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-3.5 border-t-2 border-[hsl(var(--border))]">
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            >
              <Save size={14} />
              <span>Lưu cấu hình phím tắt</span>
            </button>
          </div>
        </div>
      </div>
...
const KeyboardShortcutsCard: React.FC = () => {
  ...
  const answerKeyText =
    shortcuts.answerKeys === 'both'
      ? '1, 2, 3, 4 & A, B, C, D'
      : shortcuts.answerKeys === '1-4'
      ? '1, 2, 3, 4'
      : 'A, B, C, D';

  const nextPrevText = `${shortcuts.nextKey === 'ArrowRight' ? '→' : shortcuts.nextKey} / ${shortcuts.prevKey === 'ArrowLeft' ? '←' : shortcuts.prevKey}`;

  return (
    <>
      <Card className="rounded-3xl border-2 border-[hsl(var(--border))] shadow-xs bg-[hsl(var(--card))]">
        <CardContent className="p-5 space-y-3.5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold text-sm sm:text-base flex items-center gap-2 text-[hsl(var(--foreground))]">
                <Keyboard size={18} className="text-[hsl(var(--primary))]" /> Bảng Phím Tắt Thao Tác Nhanh
              </h2>
              <p className="text-xs text-[hsl(var(--muted-foreground))] mt-0.5">
                Các phím bấm giúp bạn làm bài trắc nghiệm với tốc độ nhanh nhất.
              </p>
            </div>
            <button
              type="button"
              onClick={() => setIsModalOpen(true)}
              className="px-3.5 py-1.5 rounded-xl text-xs font-bold border-2 border-[hsl(var(--border))] bg-[hsl(var(--muted)/0.4)] hover:bg-[hsl(var(--primary))] hover:text-white hover:border-[hsl(var(--primary))] text-[hsl(var(--foreground))] transition-all cursor-pointer shadow-2xs active:scale-95 flex items-center gap-1.5"
            >
              <Settings size={14} />
              <span>Tùy chỉnh nút ấn</span>
            </button>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 text-xs">
            {[
              [answerKeyText, 'Chọn đáp án A, B, C, D'],
              [nextPrevText, 'Câu tiếp / Câu trước'],
              [shortcuts.markKey, 'Đánh dấu câu'],
              [shortcuts.confirmKey, 'Xác nhận (luyện tập)'],
              ...
              <div
                key={desc}
                onClick={() => setIsModalOpen(true)}
                className="p-3 rounded-2xl bg-[hsl(var(--muted)/0.25)] border-2 border-[hsl(var(--border))] space-y-1 hover:border-[hsl(var(--primary)/0.4)] transition-all cursor-pointer group shadow-2xs"
                title="Bấm để tùy chỉnh phím tắt này"
              >
                <div className="flex items-center justify-between">
                  <kbd className="px-2 py-0.5 rounded-lg bg-[hsl(var(--card))] text-[11px] font-mono font-extrabold border border-[hsl(var(--border))] text-[hsl(var(--foreground))] shadow-2xs group-hover:border-[hsl(var(--primary))]">
                    {key}
                  </kbd>
                  <span className="text-[10px] text-[hsl(var(--primary))] opacity-0 group-hover:opacity-100 transition-opacity font-bold">
                    ✏️ Đổi
                  </span>
                </div>
                <p className="text-[10.5px] font-medium text-[hsl(var(--muted-foreground))] leading-tight">{desc}</p>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <ShortcutCustomizerModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </>
  );
};
...
const SoundSettingsCard: React.FC = () => {
  ...
  const handleSfxChange = (val: number) => {
    ...
    setSfxVol(clamped);
    soundscapeService.setSfxVolume(clamped / 100);
  };
  ...
  return (
    <Card className="rounded-3xl border-2 border-[hsl(var(--border))] shadow-xs bg-[hsl(var(--card))]">
      <CardContent className="p-5 space-y-4">
        ...
        <div className="space-y-3 p-4 rounded-2xl bg-[hsl(var(--muted)/0.3)] border-2 border-[hsl(var(--border))]">
          <div className="flex justify-between items-center text-xs">
            <label className="font-bold text-[hsl(var(--foreground))] flex items-center gap-2">
              {sfxVol === 0 ? <VolumeX size={16} className="text-rose-500" /> : sfxVol < 50 ? <Volume1 size={16} className="text-amber-500" /> : <Volume2 size={16} className="text-emerald-500" />}
              <span>Âm lượng hiệu ứng (SFX)</span>
            </label>
            <span className="font-mono font-black text-xs px-2.5 py-0.5 rounded-lg bg-[hsl(var(--primary)/0.15)] text-[hsl(var(--primary))] border border-[hsl(var(--primary)/0.3)] shadow-2xs">
              {sfxVol}%
            </span>
          </div>
          ...
          <div className="py-1">
            <input
              ...
              onInput={e => handleSfxChange(parseInt((e.target as HTMLInputElement).value, 10))}
              className="w-full h-3 bg-[hsl(var(--muted))] rounded-lg appearance-none cursor-pointer accent-[hsl(var(--primary))] shadow-inner"
            />
          </div>

          {/* Quick Volume Preset Buttons */}
          <div className="flex items-center gap-2 pt-0.5">
            <span className="text-[10.5px] font-semibold text-[hsl(var(--muted-foreground))]">Nhanh:</span>
            <div className="flex flex-wrap gap-1.5 flex-1">
              {[
                { label: '🔇 Tắt (0%)', val: 0 },
                { label: '🔉 30%', val: 30 },
                ...
                <button
                  key={p.val}
                  type="button"
                  onClick={() => handleSfxChange(p.val)}
                  className={`px-2.5 py-1 rounded-lg text-[10.5px] font-bold border transition-all cursor-pointer shadow-2xs ${
                    sfxVol === p.val
                      ? 'bg-[hsl(var(--primary))] text-white border-[hsl(var(--primary))] shadow-xs scale-105'
        ...
        {/* Test Sound Buttons */}
        <div className="space-y-2 pt-1">
          <span className="text-[11px] font-bold text-[hsl(var(--muted-foreground))] block">Thử âm thanh ngay:</span>
          <div className="grid grid-cols-3 gap-2.5">
            <button
              type="button"
              onClick={testCorrect}
              className="px-3 py-2 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold border-2 border-emerald-500/30 flex items-center justify-center gap-1.5 transition-all active:scale-95 cursor-pointer shadow-2xs hover:shadow-xs"
            >
            ...
            <button
              type="button"
              onClick={testWrong}
              className="px-3 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 text-xs font-bold border-2 border-rose-500/30 flex items-center justify-center gap-1.5 transition-all active:scale-95 cursor-pointer shadow-2xs hover:shadow-xs"
            >
              <span>🔊 Tiếng Sai</span>
            </button>
            ...
            <button
              type="button"
              onClick={testLevelUp}
              className="px-3 py-2 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 text-xs font-bold border-2 border-amber-500/30 flex items-center justify-center gap-1.5 transition-all active:scale-95 cursor-pointer shadow-2xs hover:shadow-xs"
            >
              <span>✨ Thăng Cấp</span>
            </button>
          </div>
