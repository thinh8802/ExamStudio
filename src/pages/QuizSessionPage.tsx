...
export const QuizSessionPage: React.FC = () => {
  ...
  // Keyboard shortcuts (Visual-mapped to A/B/C/D on screen)
  React.useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 'n': case 'N': case 'ArrowRight': nextQuestion(); break;
        case 'p': case 'P': case 'ArrowLeft': prevQuestion(); break;
        case '1': case 'a': case 'A': handleVisualAnswer(0); break;
        case '2': case 'b': case 'B': handleVisualAnswer(1); break;
        case '3': case 'c': case 'C': handleVisualAnswer(2); break;
        case '4': case 'd': case 'D': handleVisualAnswer(3); break;
        case '5': case 'e': case 'E': handleVisualAnswer(4); break;
        case '6': case 'f': case 'F': handleVisualAnswer(5); break;
        case ' ': e.preventDefault(); markQuestion(question.id); break;
        case 'Enter': if (isPractice && currentAnswer?.selectedAnswer) nextQuestion(); break;
      }
    };
    document.addEventListener('keydown', handler);
    return () => document.removeEventListener('keydown', handler);
  }, [currentIndex, currentQuestions, currentAttempt?.shuffledAnswerMap]);
  ...
  const isPractice = currentAttempt.mode === 'practice' || currentAttempt.mode === 'smart_wrong'
  ...
  const answeredCount = currentAttempt.answers.filter(a => a.selectedAnswer).length;
  const markedCount = currentAttempt.answers.filter(a => a.isMarked).length;
  const unansweredCount = currentAttempt.totalQuestions - answeredCount;
  const progressPercent = (answeredCount / currentAttempt.totalQuestions) * 100;
