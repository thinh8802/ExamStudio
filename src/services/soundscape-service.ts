...
class SoundscapeService {
  ...
  // --- Level-up Chime (Crystal Bell Sound Effect for Strike Milestones) ---
  public playLevelUpChime(tier: number = 1) {
    try {
      ...
      // Crystal frequencies ascending with tiers (Pentatonic Chimes)
      const baseFreqs = [528, 660, 792, 880, 1056, 1320, 1584];
      const rootFreq = baseFreqs[Math.min(tier - 1, baseFreqs.length - 1)] || 528;
      ...
      osc1.type = 'sine';
      osc1.frequency.setValueAtTime(rootFreq, now);
      osc1.frequency.exponentialRampToValueAtTime(rootFreq * 1.5, now + 0.35);

      osc2.type = 'triangle';
      osc2.frequency.setValueAtTime(rootFreq * 2, now);
      osc2.frequency.exponentialRampToValueAtTime(rootFreq * 2.5, now + 0.35);

      chimeGain.gain.setValueAtTime(0.0001, now);
      chimeGain.gain.linearRampToValueAtTime(0.85, now + 0.03);
      chimeGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.95);
