...
export const Modal: React.FC<ModalProps> = ({ open, isOpen, onClose, title, description, size = 'md', children, footer }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true">
      ...
      />
      <div className={cn(
        'relative z-10 w-full mx-4 bg-[hsl(var(--card))] rounded-xl shadow-[var(--shadow-xl)] border border-[hsl(var(--border))]',
        'animate-scale-in max-h-[85vh] flex flex-col',
        modalSizes[size]
      )}>
        {title && (
          <div className="p-5 pb-3 border-b border-[hsl(var(--border))]">
            {typeof title === 'string' ? (
        ...
        {footer && (
          <div className="p-5 pt-3 border-t border-[hsl(var(--border))] flex justify-end gap-2">{footer}</div>
        )}
      </div>
    </div>
  );
