...
autoUpdater.on('error', (err) => {
  ...
  let cleanMsg = err ? err.message : 'Lỗi kết nối kiểm tra cập nhật';
  if (cleanMsg.includes('not signed by the application owner') || cleanMsg.includes('certificate chain')) {
    cleanMsg = 'Lỗi xác minh chứng chỉ cập nhật. Đã bỏ qua xác minh chữ ký cục bộ.';
  }
  sendUpdaterStatus({
    status: 'error',
    message: cleanMsg,
...
function getMachineId() {
  try {
    if (process.platform === 'win32') {
      ...
      if (match && match[1]) {
        const hash = crypto.createHash('sha256').update('EPS-SALT-V1-' + match[1].trim()).digest('hex');
  ...
  try {
    const fallbackSeed = os.hostname() + '-' + os.userInfo().username + '-' + os.arch() + '-' + (os.networkInterfaces() ? Object.keys(os.networkInterfaces()).join('') : '');
    const hash = crypto.createHash('sha256').update('EPS-FALLBACK-V1-' + fallbackSeed).digest('hex');
    return `EPS-${hash.substring(0, 4).toUpperCase()}-${hash.substring(4, 8).toUpperCase()}-${hash.substring(8, 12).toUpperCase()}`;
  } catch (e) {
    return 'EPS-DEVICE-DEFAULT';
  }
}
