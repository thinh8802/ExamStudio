...
versionMap.forEach((fileList, version) => {
  ...
  // Tự động tạo bản nén .zip cho file .exe để tránh bị trình duyệt chặn tải xuống
  fileList.forEach(file => {
    if (file.endsWith('.exe')) {
      const exePath = path.join(versionFolder, file);
      const zipFileName = file.replace(/\.exe$/i, '.zip');
      const zipPath = path.join(versionFolder, zipFileName);
      if (fs.existsSync(exePath) && !fs.existsSync(zipPath)) {
        try {
          console.log(`[Release Organizer] 🗜️ Đang tự động nén ${zipFileName}...`);
          execSync(`powershell -NoProfile -Command "Compress-Archive -Path '${exePath}' -DestinationPath '${zipPath}' -Force"`, { stdio: 'pipe' });
          console.log(`[Release Organizer] ✅ Đã tạo gói nén an toàn: ${zipFileName}`);
        } catch (zipErr) {
          console.warn(`[Release Organizer] ⚠️ Không thể nén zip tự động: ${zipErr.message}`);
        }
      }
    }
  });
...
- 📁 **Thư mục Nhanh (Latest)**: \`release/latest/\`
  - \`ExamPrepStudio-Setup-${latestVersion}.exe\` (Bản Cài Đặt NSIS cho Auto Update)
  - \`latest.yml\` (Metadata cần thiết cho Auto-Updater)
  - \`ExamPrepStudio-Setup-${latestVersion}.exe.blockmap\` (Dữ liệu tải vi sai)
  - \`ExamPrepStudio-Setup-${latestVersion}.zip\` (Gói nén an toàn)

- 📁 **Thư mục Lưu Trữ**: \`release/${latestVersion}/\`

### 📚 Danh Sách Các Bản Phát Hành (${remainingVersionFolders.length} bản gần nhất):
${remainingVersionFolders.slice().reverse().map(v => `- [${v}](./${v}/): \`ExamPrepStudio-Setup-${v}.exe\``).join('\n')}
`;
