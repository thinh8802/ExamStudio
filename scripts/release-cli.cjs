      ...
      if (process.env.GH_TOKEN) {
        ...
        try {
          ...
          if (getReleaseRes.ok) {
            ...
            // Xóa file cũ nếu đã tồn tại để tránh lỗi trùng lặp khi phát hành lại cùng phiên bản
            if (relData.assets && relData.assets.length > 0) {
              for (const asset of relData.assets) {
                if (filesToUpload.includes(asset.name)) {
                  console.log(`  🗑️ Xóa file cũ trên GitHub Release: ${asset.name}...`);
                  await fetch(`https://api.github.com/repos/${repoOwner}/${repoName}/releases/assets/${asset.id}`, {
                    method: 'DELETE',
                    headers: {
                      'Authorization': `Bearer ${process.env.GH_TOKEN}`,
                      'Accept': 'application/vnd.github+json',
